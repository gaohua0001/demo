package com.dvt.snjs.dubbo.model;

import java.io.Serializable;

/**
 * 快递包裹航空费用
 * @author zane
 *
 */
public class JsYsxxHkfyDay implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1561791374545379281L;

	private String jsrq;
	private String fdid;
	private String zbtm;
	private String yjtm;
	private String sjjgs;
	private String sjjsf;
	private String pyjgs;
	private String pyjsf;
	private String yldm;
	private String yljb;
	private String ylzl;
	private String ldlsh;
	private double gls;
	private String fcz;
	private String jsz;
	private String yjzl;
	private double zl;
	private double jhzl;
	private String sjjdm;
	private String sjjds;
	private String sjjxs;
	private String pyj;
	private String pyjds;
	private String pyjxs;
	private double syfl;
	private double bcfl;
	private double syf;
	private double bcf;
	private String hbbh;
	private String zlsx;
	private String jssx;
	private double yjsl;
	private double zbzl;
	private String lhbz;
	private int id;
	private String yjjdm;
	private String jdjdm;
	private String jdjsf;
	private String ypdj;
	private String info;
	private String sjrq;
	private String jdjds;
	private String khdm;
	private String xzbz;
	private String yjly;
	
	public String getJsrq() {
		return jsrq;
	}
	public void setJsrq(String jsrq) {
		this.jsrq = jsrq;
	}
	public String getFdid() {
		return fdid;
	}
	public void setFdid(String fdid) {
		this.fdid = fdid;
	}
	public String getZbtm() {
		return zbtm;
	}
	public void setZbtm(String zbtm) {
		this.zbtm = zbtm;
	}
	public String getYjtm() {
		return yjtm;
	}
	public void setYjtm(String yjtm) {
		this.yjtm = yjtm;
	}
	public String getSjjgs() {
		return sjjgs;
	}
	public void setSjjgs(String sjjgs) {
		this.sjjgs = sjjgs;
	}
	public String getSjjsf() {
		return sjjsf;
	}
	public void setSjjsf(String sjjsf) {
		this.sjjsf = sjjsf;
	}
	public String getPyjgs() {
		return pyjgs;
	}
	public void setPyjgs(String pyjgs) {
		this.pyjgs = pyjgs;
	}
	public String getPyjsf() {
		return pyjsf;
	}
	public void setPyjsf(String pyjsf) {
		this.pyjsf = pyjsf;
	}
	public String getYldm() {
		return yldm;
	}
	public void setYldm(String yldm) {
		this.yldm = yldm;
	}
	public String getYljb() {
		return yljb;
	}
	public void setYljb(String yljb) {
		this.yljb = yljb;
	}
	public String getYlzl() {
		return ylzl;
	}
	public void setYlzl(String ylzl) {
		this.ylzl = ylzl;
	}
	public String getLdlsh() {
		return ldlsh;
	}
	public void setLdlsh(String ldlsh) {
		this.ldlsh = ldlsh;
	}
	public double getGls() {
		return gls;
	}
	public void setGls(double gls) {
		this.gls = gls;
	}
	public String getFcz() {
		return fcz;
	}
	public void setFcz(String fcz) {
		this.fcz = fcz;
	}
	public String getJsz() {
		return jsz;
	}
	public void setJsz(String jsz) {
		this.jsz = jsz;
	}
	public String getYjzl() {
		return yjzl;
	}
	public void setYjzl(String yjzl) {
		this.yjzl = yjzl;
	}
	public double getZl() {
		return zl;
	}
	public void setZl(double zl) {
		this.zl = zl;
	}
	public double getJhzl() {
		return jhzl;
	}
	public void setJhzl(double jhzl) {
		this.jhzl = jhzl;
	}
	public String getSjjdm() {
		return sjjdm;
	}
	public void setSjjdm(String sjjdm) {
		this.sjjdm = sjjdm;
	}
	public String getSjjds() {
		return sjjds;
	}
	public void setSjjds(String sjjds) {
		this.sjjds = sjjds;
	}
	public String getSjjxs() {
		return sjjxs;
	}
	public void setSjjxs(String sjjxs) {
		this.sjjxs = sjjxs;
	}
	public String getPyj() {
		return pyj;
	}
	public void setPyj(String pyj) {
		this.pyj = pyj;
	}
	public String getPyjds() {
		return pyjds;
	}
	public void setPyjds(String pyjds) {
		this.pyjds = pyjds;
	}
	public String getPyjxs() {
		return pyjxs;
	}
	public void setPyjxs(String pyjxs) {
		this.pyjxs = pyjxs;
	}
	public double getSyfl() {
		return syfl;
	}
	public void setSyfl(double syfl) {
		this.syfl = syfl;
	}
	public double getBcfl() {
		return bcfl;
	}
	public void setBcfl(double bcfl) {
		this.bcfl = bcfl;
	}
	public double getSyf() {
		return syf;
	}
	public void setSyf(double syf) {
		this.syf = syf;
	}
	public double getBcf() {
		return bcf;
	}
	public void setBcf(double bcf) {
		this.bcf = bcf;
	}
	public String getHbbh() {
		return hbbh;
	}
	public void setHbbh(String hbbh) {
		this.hbbh = hbbh;
	}
	public String getZlsx() {
		return zlsx;
	}
	public void setZlsx(String zlsx) {
		this.zlsx = zlsx;
	}
	public String getJssx() {
		return jssx;
	}
	public void setJssx(String jssx) {
		this.jssx = jssx;
	}
	public double getYjsl() {
		return yjsl;
	}
	public void setYjsl(double yjsl) {
		this.yjsl = yjsl;
	}
	public double getZbzl() {
		return zbzl;
	}
	public void setZbzl(double zbzl) {
		this.zbzl = zbzl;
	}
	public String getLhbz() {
		return lhbz;
	}
	public void setLhbz(String lhbz) {
		this.lhbz = lhbz;
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getYjjdm() {
		return yjjdm;
	}
	public void setYjjdm(String yjjdm) {
		this.yjjdm = yjjdm;
	}
	public String getJdjdm() {
		return jdjdm;
	}
	public void setJdjdm(String jdjdm) {
		this.jdjdm = jdjdm;
	}
	public String getJdjsf() {
		return jdjsf;
	}
	public void setJdjsf(String jdjsf) {
		this.jdjsf = jdjsf;
	}
	public String getYpdj() {
		return ypdj;
	}
	public void setYpdj(String ypdj) {
		this.ypdj = ypdj;
	}
	public String getInfo() {
		return info;
	}
	public void setInfo(String info) {
		this.info = info;
	}
	public String getSjrq() {
		return sjrq;
	}
	public void setSjrq(String sjrq) {
		this.sjrq = sjrq;
	}
	public String getJdjds() {
		return jdjds;
	}
	public void setJdjds(String jdjds) {
		this.jdjds = jdjds;
	}
	public String getKhdm() {
		return khdm;
	}
	public void setKhdm(String khdm) {
		this.khdm = khdm;
	}
	public String getXzbz() {
		return xzbz;
	}
	public void setXzbz(String xzbz) {
		this.xzbz = xzbz;
	}
	public String getYjly() {
		return yjly;
	}
	public void setYjly(String yjly) {
		this.yjly = yjly;
	}

	
}
