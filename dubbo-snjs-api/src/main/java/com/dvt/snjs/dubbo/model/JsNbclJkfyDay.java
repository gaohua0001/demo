package com.dvt.snjs.dubbo.model;

import java.io.Serializable;

/**
 * 内部处理进口省费用
 * 
 * @author zane
 *
 */
public class JsNbclJkfyDay implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 8077608993150649380L;

	private String jsrq;
	private String zbtm;
	private String yjtm;
	private String yjzl;
	private String clj;
	private String cljgs;
	private String cljsf;
	private String cljds;
	private String cljxs;
	private String sjj;
	private String sjjgs;
	private String sjjsf;
	private String jdj;
	private String jdjsf;
	private String jssx;
	private String zlsx;
	private double zl;
	private double jhzl;
	private double zbzl;
	private double syfl;
	private double bcfl;
	private double syf;
	private double bcf;
	private int id;
	private String sjjds;
	private String sjjxs;
	private String ypdj;
	private String time;
	private String info;
	private double syfl2;
	private double bcfl2;
	private String sjrq;
	private double ftcs;
	private String jdjds;
	private String jbbz;
	private String khdm;
	private String yjly;
	private String xzbz;
	public String getJsrq() {
		return jsrq;
	}
	public void setJsrq(String jsrq) {
		this.jsrq = jsrq;
	}
	public String getZbtm() {
		return zbtm;
	}
	public void setZbtm(String zbtm) {
		this.zbtm = zbtm;
	}
	public String getYjtm() {
		return yjtm;
	}
	public void setYjtm(String yjtm) {
		this.yjtm = yjtm;
	}
	public String getYjzl() {
		return yjzl;
	}
	public void setYjzl(String yjzl) {
		this.yjzl = yjzl;
	}
	public String getClj() {
		return clj;
	}
	public void setClj(String clj) {
		this.clj = clj;
	}
	public String getCljgs() {
		return cljgs;
	}
	public void setCljgs(String cljgs) {
		this.cljgs = cljgs;
	}
	public String getCljsf() {
		return cljsf;
	}
	public void setCljsf(String cljsf) {
		this.cljsf = cljsf;
	}
	public String getCljds() {
		return cljds;
	}
	public void setCljds(String cljds) {
		this.cljds = cljds;
	}
	public String getCljxs() {
		return cljxs;
	}
	public void setCljxs(String cljxs) {
		this.cljxs = cljxs;
	}
	public String getSjj() {
		return sjj;
	}
	public void setSjj(String sjj) {
		this.sjj = sjj;
	}
	public String getSjjgs() {
		return sjjgs;
	}
	public void setSjjgs(String sjjgs) {
		this.sjjgs = sjjgs;
	}
	public String getSjjsf() {
		return sjjsf;
	}
	public void setSjjsf(String sjjsf) {
		this.sjjsf = sjjsf;
	}
	public String getJdj() {
		return jdj;
	}
	public void setJdj(String jdj) {
		this.jdj = jdj;
	}
	public String getJdjsf() {
		return jdjsf;
	}
	public void setJdjsf(String jdjsf) {
		this.jdjsf = jdjsf;
	}
	public String getJssx() {
		return jssx;
	}
	public void setJssx(String jssx) {
		this.jssx = jssx;
	}
	public String getZlsx() {
		return zlsx;
	}
	public void setZlsx(String zlsx) {
		this.zlsx = zlsx;
	}
	public double getZl() {
		return zl;
	}
	public void setZl(double zl) {
		this.zl = zl;
	}
	public double getJhzl() {
		return jhzl;
	}
	public void setJhzl(double jhzl) {
		this.jhzl = jhzl;
	}
	public double getZbzl() {
		return zbzl;
	}
	public void setZbzl(double zbzl) {
		this.zbzl = zbzl;
	}
	public double getSyfl() {
		return syfl;
	}
	public void setSyfl(double syfl) {
		this.syfl = syfl;
	}
	public double getBcfl() {
		return bcfl;
	}
	public void setBcfl(double bcfl) {
		this.bcfl = bcfl;
	}
	public double getSyf() {
		return syf;
	}
	public void setSyf(double syf) {
		this.syf = syf;
	}
	public double getBcf() {
		return bcf;
	}
	public void setBcf(double bcf) {
		this.bcf = bcf;
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getSjjds() {
		return sjjds;
	}
	public void setSjjds(String sjjds) {
		this.sjjds = sjjds;
	}
	public String getSjjxs() {
		return sjjxs;
	}
	public void setSjjxs(String sjjxs) {
		this.sjjxs = sjjxs;
	}
	public String getYpdj() {
		return ypdj;
	}
	public void setYpdj(String ypdj) {
		this.ypdj = ypdj;
	}
	public String getTime() {
		return time;
	}
	public void setTime(String time) {
		this.time = time;
	}
	public String getInfo() {
		return info;
	}
	public void setInfo(String info) {
		this.info = info;
	}
	public double getSyfl2() {
		return syfl2;
	}
	public void setSyfl2(double syfl2) {
		this.syfl2 = syfl2;
	}
	public double getBcfl2() {
		return bcfl2;
	}
	public void setBcfl2(double bcfl2) {
		this.bcfl2 = bcfl2;
	}
	public String getSjrq() {
		return sjrq;
	}
	public void setSjrq(String sjrq) {
		this.sjrq = sjrq;
	}
	public double getFtcs() {
		return ftcs;
	}
	public void setFtcs(double ftcs) {
		this.ftcs = ftcs;
	}
	public String getJdjds() {
		return jdjds;
	}
	public void setJdjds(String jdjds) {
		this.jdjds = jdjds;
	}
	public String getJbbz() {
		return jbbz;
	}
	public void setJbbz(String jbbz) {
		this.jbbz = jbbz;
	}
	public String getKhdm() {
		return khdm;
	}
	public void setKhdm(String khdm) {
		this.khdm = khdm;
	}
	public String getYjly() {
		return yjly;
	}
	public void setYjly(String yjly) {
		this.yjly = yjly;
	}
	public String getXzbz() {
		return xzbz;
	}
	public void setXzbz(String xzbz) {
		this.xzbz = xzbz;
	}


}
